...
fn is_entertainment(app: &str, title: &str) -> bool {
    ...
    if matches_any(title, &[
        ...
        "twitch", "bilibili", "vimeo", "dailymotion",
    ]) {
        return true;
    }
    ...
    // Social media
    if matches_any(title, &[
        "reddit", "twitter", "x.com", "facebook", "instagram",
        "tiktok", "snapchat", "pinterest", "tumblr", "weibo",
...
fn is_development(app: &str, title: &str) -> bool {
    // IDEs and editors
    if matches_any(app, &[
        "code.exe", "devenv.exe", "idea64.exe", "webstorm64.exe",
        "pycharm64.exe", "goland64.exe", "clion64.exe", "rider64.exe",
        "android studio", "xcode", "sublime_text.exe", "atom.exe",
        "notepad++.exe", "cursor.exe", "windsurf.exe", "zed.exe",
    ]) {
        return true;
    }
    ...
    if matches_any(app, &[
        ...
        "wt.exe", "conhost.exe", "alacritty.exe", "hyper.exe",
    ]) {
        return true;
    }
    ...
    if matches_any(title, &[
        ...
        "localhost", "127.0.0.1", "developer.", "docs.rs", "crates.io",
...
fn is_productivity(app: &str, title: &str) -> bool {
    ...
    // Academic/writing
    if matches_any(title, &[
        "overleaf", "latex", "grammarly", "hemingway",
        "scrivener", "ulysses",
    ]) {
        return true;
    }
    ...
    if matches_any(title, &[
        ...
        "basecamp", "linear", "todoist", "ticktick",
    ]) {
        return true;
    }
...
fn is_communication(app: &str, title: &str) -> bool {
    ...
    // Video calls
    if matches_any(title, &[
        "meeting", "call", "conference", "webinar",
    ]) {
        return true;
    }
...
mod tests {
    ...
    #[test]
    fn test_entertainment_netflix() {
        assert_eq!(
            categorize_heuristic("chrome.exe", "Netflix - Stranger Things"),
            Category::Entertainment
        );
    }

    #[test]
    fn test_entertainment_reddit() {
        assert_eq!(
            categorize_heuristic("firefox.exe", "reddit: the front page of the internet"),
            Category::Entertainment
        );
    }
    ...
    #[test]
    fn test_development_github() {
        assert_eq!(
            categorize_heuristic("msedge.exe", "GitHub - anthropics/claude-code"),
            Category::Development
        );
    }

    #[test]
    fn test_development_terminal() {
        assert_eq!(
            categorize_heuristic("WindowsTerminal.exe", "PowerShell"),
            Category::Development
        );
    }
    ...
    #[test]
    fn test_productivity_notion() {
        assert_eq!(
            categorize_heuristic("notion.exe", "My Workspace - Notion"),
            Category::Productivity
        );
    }

    #[test]
    fn test_productivity_google_docs() {
        assert_eq!(
            categorize_heuristic("chrome.exe", "Untitled document - Google Docs"),
            Category::Productivity
        );
    }
    ...
    #[test]
    fn test_communication_gmail() {
        assert_eq!(
            categorize_heuristic("chrome.exe", "Inbox - Gmail"),
            Category::Communication
        );
    }

    #[test]
    fn test_communication_teams() {
        assert_eq!(
            categorize_heuristic("teams.exe", "Microsoft Teams"),
            Category::Communication
        );
    }
    ...
    #[test]
    fn test_create_heuristic_label() {
        let label = create_heuristic_label(
            "abc123",
            "msedge.exe",
            "YouTube - Video",
        );
        assert_eq!(label.title_hash, "abc123");
        assert_eq!(label.category, "entertainment");
        assert_eq!(label.source, LabelSource::Heuristic);
